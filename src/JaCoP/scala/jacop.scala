...
class IntSet extends JaCoP.core.IntervalDomain {
   ...
   */
  def unary_~ : IntSet = {
    var tmp = new IntSet(JaCoP.core.IntDomain.MinInt, JaCoP.core.IntDomain.MaxInt)
 ...
 */
class IntVar(name: String, min: Int, max: Int) extends JaCoP.core.IntVar(Model, name, min, max) with jacop {
 ...
 */
class SetVar(name: String, glb: Int, lub: Int) extends JaCoP.set.core.SetVar(Model, name, glb, lub) {
 ...
 */
class BoolVar(name: String, min1: Int, max1: Int) extends JaCoP.core.BooleanVar(Model, name, min1, max1)
  with jacop {
...
//class XgtC(name: IntVar, const: Int) extends JaCoP.constraints.XgtC(name, const) {
//  def <=>(that: IntVar) = {
...
class network extends JaCoP.constraints.netflow.NetworkBuilder {
  ...
  var nodes = HashMap[node, JaCoP.constraints.netflow.simplex.Node]()
   ...
   * @param n node
   */
  def apply(n: node): JaCoP.constraints.netflow.simplex.Node = {
    return nodes(n)
  }
 ...
 */
case class node(var name: String, var balance: Int) { //extends JaCoP.constraints.netflow.simplex.Node(name, balance) {
